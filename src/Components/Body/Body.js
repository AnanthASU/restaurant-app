...
function RestaurantBody() {
  ...
  const {
    ...
    "https://cors-anywhere.herokuapp.com/https://www.swiggy.com/api/seo/getListing?lat=13.0169992&lng=77.7044335",
    options
  );
    ...
    tempData != undefined &&
      setFilteredData(
        tempData.filter((item) =>
          item.info.name
            .toLocaleUpperCase()
            .includes(debouncedSearchTerm?.toLocaleUpperCase())
        )
      );
  };
  ...
  React.useEffect(() => {
    const newData =
      rData?.data?.success?.cards[0]?.card?.card?.gridElements?.infoWithStyle
        ?.restaurants;
    newData != undefined &&
      setResdata((prevData) => {
        if (!prevData || !Array.isArray(prevData)) {
          ...
          const uniqueNewData = newData.filter(
            (newItem) =>
              !prevData.some((prevItem) => prevItem.info.id === newItem.info.id)
          );
  ...
  const HandleSort = (id) => {
    if (id == 1) {
      setFilteredData((prev) => {
        const SortedArray = [...prev].sort(
          (a, b) =>
            getNumericRating(a.info.sla.slaString) -
            getNumericRating(b.info.sla.slaString)
        );
        return SortedArray;
      });
    }
    if (id === 2) {
      setFilteredData((prev) => {
        const SortedArray = [...prev].sort(
          (a, b) =>
            getNumericRating(b.info.avgRating) -
            getNumericRating(a.info.avgRating)
        );
        return SortedArray;
      });
    }
  ...
  return (
    <div
      className="body-wraper pt-60 pb-10"
      style={{ paddingLeft: "13%", paddingRight: "13%" }}
    >
      <div>
        <BannerItems />
      </div>
      <div className="border-solid border-2 border-gray-300 my-6" />
      <div className="font-bold flex text-2xl p-4">
      ...
      <div className="custmoizations p-3 py-10">
        <div className=" bg-gray-100 search-bar flex items-center pl-4 pr-10 rounded-3xl">
          ...
          <input
            ...
            value={searchTerm}
            className=" bg-gray-100 search-input focus:outline-none flex-grow"
          />
        </div>
